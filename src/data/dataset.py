...
class KnowledgeDataset(Dataset):
    ...
    def __init__(self, sent, knwl, pos, params):
        super(KnowledgeDataset, self).__init__(sent, pos, params)
        self.k_pad_index = params.pad_index
        self.k_eos_index = params.eos_index
        self.knwl = knwl

    def batch_knowledge(self, knowledge_sentences):
        """
        # Take as input a list of n sentences (torch.LongTensor vectors) and return
        # a tensor of size (slen, n) where slen is the length of the longest
        # sentence, and a vector lengths containing the length of each sentence.
        knowledge_sentences is (batch_size, sentence_size, entity_list_length)
        knowledge_sentences[0] is a sequence of knowledge lists
        knowledge_sentences[0, 0] is a list of knowledge related to a word in the sentence
        knowledge_sentences[0, 0, 0] is an entity id, dtype=Long
        """
        # # sentences = sorted(sentences, key=lambda x: len(x), reverse=True)
        # lengths = torch.LongTensor([len(s) + 2 for s in sentences])
        # sent = torch.LongTensor(lengths.max().item(), lengths.size(0)).fill_(self.pad_index)
        try:
            k_length = max(max(len(k_list) for k_list in k_sent) for k_sent in knowledge_sentences)
        except ValueError:
            return None
        s_length = max(len(s) for s in knowledge_sentences)
        # size of k_lengths: (batch, sentence)
        try:
            k_lengths = [[len(k_list) for k_list in k_sent] for k_sent in knowledge_sentences]
            for k_l in k_lengths:
                if len(k_l) < s_length:
                    k_l.extend([0] * (s_length - len(k_l)))
            k_lengths = torch.LongTensor(k_lengths)
        except:
            pass
        # size of s_lengths: (batch)
        s_lengths = torch.LongTensor([len(k_sent) for k_sent in k_lengths])
        knwl_list = []
        for k_sent in knowledge_sentences:
            k_list_batch = [k_list + [self.k_pad_index] * (k_length - len(k_list)) for k_list in k_sent]
            k_list_batch += [[self.k_pad_index] * k_length] * (s_length - len(k_list_batch))
            knwl_list.append(k_list_batch)
        knwl = torch.LongTensor(knwl_list)
        ...
        #
        # return sent, lengths
        return knwl, s_lengths, k_lengths

    def remove_empty_sentences(self):
        indices = super().remove_empty_sentences()
        self.knwl = [self.knwl[i] for i in indices]
    ...
    def remove_short_sentences(self, min_len):
        indices = super().remove_short_sentences(min_len)
        self.knwl = [self.knwl[i] for i in indices]

    def select_data(self, a, b):
        super().select_data(a, b)
        self.knwl = self.knwl[a:b]

    def get_batches_iterator(self, batches, return_indices):
        gen = super().get_batches_iterator(batches, True)
        while True:
            try:
                sent, sentence_ids = next(gen)
            except StopIteration:
                return
            knwl = [self.knwl[id_] for id_ in sentence_ids]
            knwl = self.batch_knowledge(knwl)
            yield (sent, knwl, sentence_ids) if return_indices else (sent, knwl)
