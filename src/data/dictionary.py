...
from .knowledge import attach_wordnet_bpe
...
class KnowledgeDictionary:
    ...
    def __init__(self, knwl2id):
        self.knwl2id = knwl2id
        self.pad_index = knwl2id[PAD_KNWL]

    def __contains__(self, w):
        """
        Returns whether a word is in the dictionary.
        """
        return w in self.knwl2id
        
    def index(self, word):
        """
        Returns the index of the specified word.
        """
        return self.knwl2id[word]
    
    @staticmethod
    def read_knowledge(name2id_path):
        import pickle
        with open(name2id_path, 'rb') as f:
            name2id = pickle.load(f)
        k_dico = KnowledgeDictionary(name2id)
        return k_dico
...
class Dictionary(object):
    ...
    def __init__(self, id2word, word2id, counts, knwl2id=None):
        ...
        if knwl2id is not None:
            self.knwl2id = knwl2id
            self.k_bos_index = knwl2id[BOS_WORD]
            self.k_eos_index = knwl2id[EOS_WORD]
            self.k_pad_index = knwl2id[PAD_WORD]
        self.check_valid()
    ...
    @staticmethod
    def read_vocab(vocab_path, name2id_path=None):
        """
        ...
        dico = Dictionary(id2word, word2id, counts, name2id if name2id_path is not None else None)
    ...
    @staticmethod
    def index_data(path, bin_path, dico, k_dico=None, knwl2id=None):
        """
        ...
        if bin_path is not None and os.path.isfile(bin_path):
            ...
            assert (k_dico is None and knwl2id is None and 'k_dico' not in data) or (k_dico == data['k_dico'] and knwl2id is not None)
            return data
        ...
        if k_dico is not None:
            lang = bin_path[-6:-4]
            f.seek(0)
            lines = f.read()
            k_sents = attach_wordnet_bpe(lines, dico.knwl2id, lang=lang)
        f.close()
