...
from .dataset import Dataset, StreamDataset, ParallelDataset, KnowledgeDataset
...
def set_dico_parameters(params, data, dico):
    ...
    if 'k_dico' in data:
        k_dico = data['k_dico']
        k_pad_index = k_dico.index(PAD_WORD)
        if hasattr(params, 'k_pad_index'):
            assert params.k_pad_index == k_pad_index
        else:
            params.k_pad_index = k_pad_index
...
def load_mono_data(params, data):
    ...
    # load img
    if params.use_image:
        data['img'] = np.load(params.img_feature_file)
        data['img_idx'] = {}
    ...
    for lang in params.mono_dataset.keys():
        ...
        if params.use_image:
            path = os.path.join(params.data_path, f'idx.{lang}')
            with open(path) as f:
                data['img_idx'][lang] = np.array(list(map(int, f.readlines())), dtype=np.int)
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            if lang in params.ae_steps or lang in params.bt_src_langs or lang in params.mass_steps:
                ...
                if params.knowledge != 'none':
                    if 'knowledge' in mono_data:
                        dataset = KnowledgeDataset(mono_data['sentences'], mono_data['knowledge'], mono_data['positions'], params)
                    else:
                        logger.error('no knowledge data')
                else:
...
def check_data_params(params):
    ...
    assert (not params.use_image) or os.path.isfile(params.img_feature_file)
