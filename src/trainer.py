...
class Trainer(object):
    ...
    def __init__(self, data, params):
        ...
        self.intermediate_l1 = torch.nn.L1Loss(reduction='mean')
    ...
    def get_optimizer_fp(self, module):
        ...
        optimizer = get_optimizer(module.parameters(), self.params.optimizer)
        if self.params.fp16:
            module, optimizer = apex.amp.initialize(module, optimizer, opt_level="O1")
    ...
    def optimize(self, loss, modules):
        ...
        if self.params.fp16:
            ...
            with apex.amp.scale_loss(loss, [self.optimizers[m] for m in modules]) as scaled_loss:
        ...
        if self.params.clip_grad_norm > 0:
            for module in modules:
                if self.params.fp16:
                    ...
                    clip_grad_norm_(apex.amp.master_params(self.optimizers[module]), self.params.clip_grad_norm)
                else:
    ...
    def save_best_model(self, scores):
        ...
        for metric, value in scores.items():
            if metric == 'epoch':
                continue
            if 'ppl' in metric or 'acc' in metric:
                #factor = -1
                continue
            else:
                factor = 1
            if factor * value > factor * self.best_metrics.get(metric, factor * float('-inf')):
                self.best_metrics[metric] = value
                logger.info('New best score for %s: %.6f' % (metric, value))
...
class SingleTrainer(Trainer):
    ...
    def __init__(self, model, data, params):
        ...
        # optimizers
        self.model, optimizer = self.get_optimizer_fp(model)
        self.optimizers = {'model': optimizer}
...
class EncDecTrainer(Trainer):
    ...
    def __init__(self, encoder, decoder, data, params):
        ...
        # optimizers
        self.encoder, enc_opt = self.get_optimizer_fp(encoder)
        self.decoder, dec_opt = self.get_optimizer_fp(decoder)
        self.optimizers = {
            'encoder': enc_opt,
            'decoder': dec_opt,
        }
    ...
    def bt_step(self, lang1, lang2, lang3, lambda_coeff):
        ...
        (x1, len1), _ = self.get_batch('bt', lang1)
        langs1 = x1.clone().fill_(lang1_id)
    ...
    def mass_step(self, lang, lambda_coeff):
        ...
        batch = self.get_batch('mass', lang)
        if use_k:
            (x_, len_), k_, x_idx = batch
        else:
            (x_, len_), x_idx = batch
        if params.use_image:
            img_idx = self.data['img_idx'][lang][np.array(x_idx)]
            img = self.data['img'][np.array(img_idx)]
            img = torch.tensor(img, device='cuda').requires_grad_(False)
            if not params.fp16:
                img = img.to(torch.float)
        else:
            img = None
        if params.knowledge == 'none' or k_ is None:
            klg = None
        else:
            klg = tuple(map(lambda t: t.cuda(), k_))
        ...
        enc1, (pre, post) = self.encoder('fwd', x=x1, lengths=len1, langs=langs1, causal=False, img=img, klg_triple=klg, return_intermediate=True)
        enc1 = enc1.transpose(0, 1)
        
        enc_mask = x1.ne(params.mask_index)
        enc_mask = enc_mask.transpose(0, 1)
        
        dec2 = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, img=img,
                            src_enc=enc1, src_len=len1, positions=positions, enc_mask=enc_mask)
        
        _, loss = self.decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y, get_scores=False)
        self.stats[('MA-%s' % lang)].append(loss.item())

        intermediate_loss = torch.zeros([], dtype=pre[0].dtype, device='cuda')
        for p1, p2 in zip(pre, post):
            intermediate_loss += self.intermediate_l1(p1, p2.detach())
        intermediate_loss /= len(pre)

        self.stats['INTER'].append(intermediate_loss.item())
        loss += params.inter_lambda * intermediate_loss
    ...
    def caption_step(self):
        assert self.params.use_image
        self.encoder.train()
        self.decoder.train()
        img = self.data['img'][np.array(img_idx)]
