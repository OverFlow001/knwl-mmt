...
#
N_MONO=9000  # number of monolingual sentences for each language
...
if [ "$SRC" == "en" -a "$TGT" == "fr" ]; then
  ...
  PARA_TGT_VALID=$PARA_PATH/dev/val.fr
  PARA_SRC_TEST=$PARA_PATH/dev/test2016.en
  PARA_TGT_TEST=$PARA_PATH/dev/test2016.fr
fi
if [ "$SRC" == "de" -a "$TGT" == "en" ]; then
  ...
  PARA_SRC_VALID=$PARA_PATH/dev/val.de
  PARA_TGT_VALID=$PARA_PATH/dev/val.en
...
# cp image idx file
cp $MONO_PATH/$SRC/idx.$SRC $MONO_PATH/$TGT/idx.$TGT $PROC_PATH

NAME2ID_PKL="./WN18RR/name2id.pkl"

# binarize data
if ! [[ -f "$SRC_TRAIN_BPE.pth" ]]; then
  echo "Binarizing $SRC data..."
  $MAIN_PATH/preprocess.py $FULL_VOCAB $SRC_TRAIN_BPE $NAME2ID_PKL
fi
if ! [[ -f "$TGT_TRAIN_BPE.pth" ]]; then
  echo "Binarizing $TGT data..."
  $MAIN_PATH/preprocess.py $FULL_VOCAB $TGT_TRAIN_BPE $NAME2ID_PKL
fi
